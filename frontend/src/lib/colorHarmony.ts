...
function skinToneScore(hex: string): number {
  ...
  if (hsl.h >= 40 && hsl.h <= 65 && hsl.s > 50) return 40;
  if (hsl.h >= 25 && hsl.h <= 40 && hsl.s > 60) return 50;
  if (hsl.h >= 180 && hsl.h <= 300) return 95;
  if (hsl.s < 15) return 85;
...
export function computeHarmony(hoodieHex: string, shortsHex: string): HarmonyResult {
  ...
  if (normalizedDiff <= 30) { harmonyType = 'analogous'; baseScore = 78; }
  else if (normalizedDiff >= 150 && normalizedDiff <= 210) { harmonyType = 'complementary'; baseScore = 88; }
  else if (normalizedDiff >= 110 && normalizedDiff <= 140) { harmonyType = 'triadic'; baseScore = 82; }
  else if (normalizedDiff >= 60 && normalizedDiff <= 90) { harmonyType = 'split-complementary'; baseScore = 75; }
  
  if (normalizedDiff < 10 && Math.abs(hsl1.l - hsl2.l) > 20) { harmonyType = 'monochrome'; baseScore = 85; }
  ...
  return { score: finalScore, type: harmonyType, explanation: explanations[harmonyType] || 'Nice color combination' };
}
...
export function generateStyleExplanation(hoodieHex: string, shortsHex: string, harmonyResult: HarmonyResult, dayOfWeek: string): string {
  ...
  return hypeLines[Math.floor(Math.random() * hypeLines.length)] + ' ' + motivational[Math.floor(Math.random() * motivational.length)];
}
